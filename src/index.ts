...
function makeRequest(verb: string, url: string) {
  ...
  const send = async function (requestData?: any) {
    try {
      ...
      const payload = serializer ? serializer(requestData) : requestData
      ...
      if (payload) {
        if (typeof payload === 'string' || Buffer.isBuffer(payload)) {
          ...
          throw new BadRequestError(
            `Request payload must be one of type string or Buffer. Received type ${typeof payload}`
          )
        }
      }
...
function createHttpRequest(
  ...
  const response: Promise<CustomResponse> = new Promise((resolve, reject) => {
    resolver = resolve
    rejector = reject
  })
...
function collectResponse(
  ...
  /**
   * Assume "utf-8" if no compression is used.
   */
  ...
  res.on('end', () => {
    ...
    if (encoding === 'gzip') {
      ...
      resData = zlib.gunzipSync(buffer)
    } else if (encoding === 'deflate') {
      const buffer = Buffer.concat(chunks as Buffer[])
      resData = zlib.inflateSync(buffer)
    } else {
      resData = Buffer.from((chunks as string[]).join(''))
    }
    callback({
      internal: res,
      buffer: resData,
 ...
 * @param url
 */
function createClientRequest(verb: string, url: string): http.ClientRequest {
  const structuredUrl = new URL(url)
